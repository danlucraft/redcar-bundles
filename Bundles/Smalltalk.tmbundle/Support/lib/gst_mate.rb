...
class GstScript < UserScript
  def lang; "Smalltalk" end
  ...
  #   cmd
  # end
  def executable; @hashbang || ENV['TM_GST'] || 'gst' end
  def version_string
    path = ENV['PATH'].split(':').find { |e| File.exists? File.join(e, executable) }
    gst_path = File.join(path.to_s, executable)
...
script = GstScript.new(STDIN.read)
ScriptMate.new(script).emit_html
